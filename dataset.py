...
def downsample(img, scale, shape=None):
    if shape is None:
        img_d = cv2.resize(img.astype('uint8'),
                           (int(scale * img.shape[1]),
                            int(scale * img.shape[0])),
    ...
    if img.shape[-1] > 3:
        if shape is None:
            img_d[:, :, 3] = cv2.resize(
                img[:, :, 3], (int(scale * img.shape[1]), int(scale * img.shape[0]))) / scale
        else:
            img_d[:, :, 3] = cv2.resize(
                img[:, :, 3], (shape[1], shape[0])) / scale
    return img_d
...
class DataSetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.scale == 2:
            ...
            img_hr_right = np.concatenate(
                load_mono_cam(1, 2), axis=-1)[..., :3]
        ...
        if self.to_tensor:
            ...
            return toTensor(img_hr_left), toTensor(img_hr_right), toTensor(
                img_lr_left), toTensor(img_lr_right)
...
def toTensor(img):
    if len(img.shape) == 4:
        ...
        if img.shape[1] > 3:
            img = torch.stack([img[:, i] / (255.0 if i != 3 else 1.0)
                              for i in range(img.shape[1])], dim=1)
        else:
            img = img.float() / 255.0
    else:
        ...
        if img.shape[0] > 3:
            img = torch.stack([img[i] / (255.0 if i != 3 else 1.0)
                              for i in range(img.shape[0])], dim=0)
        else:
            img = img.float() / 255.0
    return img
