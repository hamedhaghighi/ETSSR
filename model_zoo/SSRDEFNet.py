...
class PAM(nn.Module):
    ...
    def forward(self, x_left, x_right, fea_left, fea_right, cost, is_training):
        ...
        x_leftT = torch.bmm(M_right_to_left, x_right.permute(0, 2, 3, 1).contiguous().view(-1, w, c)).contiguous().view(b, h, w, c).permute(0, 3, 1, 2)                           #  B, C0, H0, W0
        x_rightT = torch.bmm(M_left_to_right, x_left.permute(0, 2, 3, 1).contiguous().view(-1, w, c)).contiguous().view(b, h, w, c).permute(0, 3, 1, 2)                              #  B, C0, H0, W0
...
class PAB(nn.Module):
    ...
    def flops(self, H, W):
        N = H * W
        flops = 0
        flops += 2 * (conv_flop(N, self.channels, self.channels//2, 3) + conv_flop(N, self.channels//2, self.channels, 3))
        flops += 2 * conv_flop(N, self.channels, self.channels, 1)
