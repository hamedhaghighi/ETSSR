...
def parse_args():
    ...
    parser.add_argument('--testset_dir', type=str, default='/home/haghig_h@WMGDS.WMG.WARWICK.AC.UK/Phd_datasets/iPASSR')
    parser.add_argument('--scale_factor', type=int, default=2)
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints')
    parser.add_argument('--exp_name', type=str, default='orig')
    parser.add_argument('--patch_based', default=False, action='store_true')
...
def test(cfg):
    ...
    for env in os.listdir(cfg.testset_dir):
        env_path = os.path.join(cfg.testset_dir, env)
        for idx, image_folder in enumerate(os.listdir(env_path)):
            file_path = os.path.join(env_path, image_folder,'im0.npy')
            HR_left = np.load(file_path)[...,:3].astype('uint8')
            file_path = os.path.join(env_path, image_folder, 'im1.npy')
            HR_right = np.load(file_path)[..., :3].astype('uint8')
            LR_left = downsample(HR_left)
            LR_right = downsample(HR_right)
            h, w, _ = LR_left.shape
            h_patch = biggest_divisior(h) if cfg.patch_based else h
            w_patch = biggest_divisior(w) if cfg.patch_based else w
            ...
            print('env:', env, 'id:', idx, 'psnr_left:', psnr_left, 'psnr_right:', psnr_right)
            print('env:', env, 'id:', idx, 'ssim_left:', ssim_left, 'ssim_right:', ssim_right)
            plt.figure(0)
            plt.imshow(sr_right)
            plt.figure(1)
            plt.imshow(HR_right)
