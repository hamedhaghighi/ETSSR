...
def test(cfg):
    ...
    test_set = Subset(total_dataset, range(len(total_dataset))[:len(total_dataset)//100])
    ...
    for idx in tqdm.tqdm(range(len(test_set))):
        ...
        with torch.no_grad():
            SR_left, SR_right = net(toTensor(LR_left).unsqueeze(0).to(cfg.device), toTensor(LR_right).unsqueeze(0).to(cfg.device), is_training=0)
            SR_left, SR_right = torch.clamp(SR_left, 0.0, 1.0), torch.clamp(SR_right, 0.0, 1.0)
    ...
    print('psnr_left:%.3f'% np.array(psnr_left).mean(), 'psnr_right:%.3f' % np.array(psnr_right_list).mean())
    print('ssim_left:%.3f'% np.array(ssim_left_list).mean(), 'ssim_right:%.3f'% np.array(ssim_right_list).mean())
