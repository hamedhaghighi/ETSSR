...
def downsample(img, scale):
    if scale > 1:
        if len(img.shape) == 3:
            img = cv2.resize(img,
                             (int((1 / scale) * img.shape[1]),
                              int(1 / scale * img.shape[0])),
                             interpolation=cv2.INTER_CUBIC)
...
root_dir = os.path.join(
    '/home/haghig_h@WMGDS.WMG.WARWICK.AC.UK/Phd_datasets/iPASSR/data/AirSim2',
    environment)
...
for idx in range(100):
    ...
    for i in range(2):
        cam_i_req = [airsim.ImageRequest(str(i), airsim.ImageType.Scene, False, False),
                     airsim.ImageRequest(
            str(i), airsim.ImageType.DepthPlanar, True, False),
            airsim.ImageRequest(
            str(i), airsim.ImageType.SurfaceNormals, False, False),
            airsim.ImageRequest(
            str(i), airsim.ImageType.Segmentation, False, False)
        ]
        ...
        for j in range(4):
            ...
            if response.pixels_as_float:
                response = np.expand_dims(
                    airsim.get_pfm_array(response), axis=-1)
    ...
    for scale in [1, 2, 4]:
        for n_cam in [0, 1]:
            ...
            filename = os.path.join(
                img_path, 'imgx{}_{}.png'.format(
                    scale, n_cam))
            save_sensor_data(filename, downsample(
                LandR_list[n_cam][..., :3], scale))
            filename = os.path.join(
                img_path, 'imgx{}_disp_{}'.format(
                    scale, n_cam))
            save_sensor_data(filename, downsample(
                LandR_list[n_cam][..., 3], scale))
            filename = os.path.join(
                img_path, 'imgx{}_sn_{}.png'.format(
                    scale, n_cam))
            save_sensor_data(filename, downsample(
                LandR_list[n_cam][..., 4:7], scale))
            filename = os.path.join(
                img_path, 'imgx{}_seg_{}.png'.format(
                    scale, n_cam))
            save_sensor_data(filename, downsample(
                LandR_list[n_cam][..., 7:], scale))
