...
dataset = 'Carla'
root_dir = '/home/haghig_h@WMGDS.WMG.WARWICK.AC.UK/Phd_datasets/iPASSR/data/{}'.format(
    dataset)
dest_dir = '/home/haghig_h@WMGDS.WMG.WARWICK.AC.UK/Phd_datasets/iPASSR/data/{}_patches_L'.format(
    dataset)
            ...
            for x_x4 in range(2, img_x4.shape[0] - (h_patch + 2), stride_h):
                for y_x4 in range(
                    ...
                    def save_mono_cam(img, scale):
                        ...
                        if dataset == 'AirSim':
                            filename = os.path.join(
                                dst_img_folder, 'imgx{}_sn_{}.png'.format(
                                    scale, n_cam))
