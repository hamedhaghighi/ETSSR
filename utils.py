...
def check_disparity(LR_left, LR_right):
    ...
    coords_h, coords_w = torch.meshgrid([torch.arange(h), torch.arange(w)], indexing='ij')
...
def disparity_alignment(d_left, d_right, b :int, h:int, w:int):
    ...
    coords_b, coords_h, r2l_w, l2r_w = torch.zeros(b, h, w), torch.zeros(b, h, w), torch.zeros(b, h, w), torch.zeros(b, h, w)
    return coords_b.long(), coords_h.long(), r2l_w.long(), l2r_w.long()
