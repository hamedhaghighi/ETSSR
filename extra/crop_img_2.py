...
def copy_from_checkpoints_to_all_results(
        search_name, dir, exp_names, dst_name=None):

    dst_dir = os.path.join(
        dir,
        'all_results_2/' +
        search_name) if dst_name is None else os.path.join(
        dir,
        'all_results_2/' +
        dst_name)
    ...
    for exp_name in exp_names:
        ...
        for filename in os.listdir(exp_result_dir):
            if 'Bicubic' in exp_name:
                search_name_t = exp_name.split('_')[
                    1] + '_' + search_name + '.' if 'Bicubic_hr' in exp_name else exp_name.split('_')[1] + '_' + search_name + '_'
                if search_name_t in filename:
                    ...
                    shutil.copy(
                        os.path.join(
                            exp_result_dir, filename), os.path.join(
...
def find_best_result_img(dir, exp_names):
    ...
    for img_filename in os.listdir(main_exp_result_dir):
        ...
        img_psnr_diff_dict[search_name + '_' +
                           min_exp_name] = math.floor(min_diff * 1000) / 1000
    img_psnr_diff_dict = {
        k: v for k,
        v in sorted(
            img_psnr_diff_dict.items(),
        ...
        for exp_name in exp_names:
            for img_filename in os.listdir(dir):
                if exp_name in img_filename and 'Cropped_' in img_filename and view in img_filename:
                    ...
                    # ax.set_aspect('equal')
                    psnr = float(
                        img_filename.split('_')[6]) if exp_name != 'HR' else 0
                    ssim = float(
                        img_filename.split('_')[7]) if exp_name != 'HR' else 0
                    ...
                    if view == 'left':
                        ...
                        title = '{:.2f}'.format((psnr + psnr_list[i - n]) / 2) + ', ' + '{:.3f}'.format(
                            (ssim + ssim_list[i - n]) / 2) if exp_name != 'HR' else ''
                        # plt.title(exp_name + '\n' + title, y =-0.01)
                        axs[i].set_xlabel(
                            exp_name if exp_name != 'Bicubic' else 'LR', fontsize=f_size)
                    # plt.axis('off')
    ...
    if 'plot' in option:
        ...
        if 'crop_images' in option:
            ...
            copy_from_checkpoints_to_all_results(
                filename_2, dir, exp_names, filename)
            dir = '../checkpoints/all_results_2/'

            crop_imgs_in_a_folder(
                dir, filename, left, top, right, bottom, 'left')
            crop_imgs_in_a_folder(dir,
                                  filename,
                                  left_1,
                                  top_1,
                                  left_1 + (right - left),
