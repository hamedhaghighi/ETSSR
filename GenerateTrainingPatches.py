...
idx_patch = 1
match_data_list = []
# Get list of scenes in Milddlebury's stereo training dataset and iterate
# through them
for scene_info in Dataset.get_training_scene_list():
    ...
    img_lr_0 = cv2.resize(img_hr_0,
                          (int(0.5 * img_hr_0.shape[1]),
                           int(0.5 * img_hr_0.shape[0])),
                          interpolation=cv2.INTER_CUBIC)
    img_lr_1 = cv2.resize(img_hr_1,
                          (int(0.5 * img_hr_1.shape[1]),
                           int(0.5 * img_hr_1.shape[0])),
                          interpolation=cv2.INTER_CUBIC)
    for x_lr in range(2, img_lr_0.shape[0] - 32, 20):
        for y_lr in range(2, img_lr_0.shape[1] - 92, 20):
            x_hr = x_lr * scale
            y_hr = y_lr * scale
            hr_patch_0 = img_hr_0[x_hr: (
                x_lr + 30) * scale, y_hr: (y_lr + 90) * scale]
            hr_patch_1 = img_hr_1[x_hr: (
                x_lr + 30) * scale, y_hr: (y_lr + 90) * scale]
            ...
            cv2.imwrite(dst_root +
                        '/hr0.png'.format(scale, idx_patch), hr_patch_0)
            cv2.imwrite(dst_root +
                        '/hr1.png'.format(scale, idx_patch), hr_patch_1)
            cv2.imwrite(dst_root +
                        '/lr0.png'.format(scale, idx_patch), lr_patch_0)
            cv2.imwrite(dst_root +
                        '/lr1.png'.format(scale, idx_patch), lr_patch_1)
