...
class Net(nn.Module):
    def __init__(self, upscale_factor, img_size, model,
                 input_channel=3, w_size=8, embed_dim=64):
        super(Net, self).__init__()
        if 'light' in model:
            from models.LightCoSwinTransformer import CoSwinAttn
        ...
        if 'IF' in self.model:
            self.init_feature = nn.Conv2d(
                2 * input_channel, embed_dim, 3, 1, 1, bias=True)
        else:
            self.init_feature = nn.Conv2d(
                input_channel, embed_dim, 3, 1, 1, bias=True)
        depths = [4]
        num_heads = [4]
        ...
        if 'swin_pam' in self.model:
            self.pre_pam = nn.Conv2d(
                embed_dim, 4 * embed_dim, 3, 1, 1, bias=True)
    ...
    def forward(self, x_left, x_right):
        ...
        if 'IF' in self.model:
            x_left = x_right = torch.cat(
                [x_left[:, :3], x_right[:, :3]], dim=1)
        else:
            x_left = x_left[:, :3]
            x_right = x_right[:, :3]
        # init feature
        ...
        if 'EF' in self.model:
            buffer_leftT, buffer_rightT = self.co_feature(
                buffer_left, buffer_right, d_left, d_right)
        # Deep Feature extraction
        buffer_left = self.deep_feature(
            buffer_leftT if 'EF' in self.model else buffer_left, CF_left)
        buffer_right = self.deep_feature(
            buffer_rightT if 'EF' in self.model else buffer_right, CF_right)
        # Cross View Fusion
        if 'EF' not in self.model and 'LF' not in self.model:
            if 'swin_pam' in self.model:
                buffer_leftT, buffer_rightT = self.pre_pam(
                    buffer_left), self.pre_pam(buffer_right)
                buffer_leftT, buffer_rightT = self.co_feature(
