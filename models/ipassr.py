...
class Net(nn.Module):
    ...
    def forward(self, x_left, x_right, is_training):
        ...
        if is_training == 0:
            buffer_leftT, buffer_rightT = self.pam(
                buffer_left, buffer_right, catfea_left, catfea_right, is_training)

        buffer_leftF = self.fusion(
            torch.cat([buffer_left, buffer_leftT], dim=1))
        buffer_rightF = self.fusion(
        ...
        if is_training == 1:
            return out_left, out_right, (M_right_to_left,
                                         M_left_to_right), (V_left, V_right)
        if is_training == 0:
    ...
    def calc_loss(self, LR_left, LR_right, HR_left, HR_right, cfg):
        ...
        ''' Photometric Loss '''
        Res_left = torch.abs(HR_left - F.interpolate(
            LR_left[:, :3], scale_factor=scale, mode='bicubic', align_corners=False))
        ...
        Res_leftT = torch.bmm(
            ...
            Res_right.permute(
                ...
                c)).view(
                    ...
                    h,
                    w,
                    c).contiguous().permute(
                        0,
                        3,
                        1,
            2)
        Res_rightT = torch.bmm(
            ...
            Res_left.permute(
                ...
                c)).view(
                    ...
                    c).contiguous().permute(
                        ...
                        1,
            2)
        self.loss_photo = criterion_L1(Res_left * V_left.repeat(1,
                                                                3,
                                                                1,
                                                                1),
                                       Res_leftT * V_left.repeat(1,
                                                                 3,
                                                                 1,
                                                                 1)) + criterion_L1(Res_right * V_right.repeat(1,
                                                                                                               3,
                                                                                                               1,
                                                                                                               1),
                                                                                    Res_rightT * V_right.repeat(1,
                                                                                                                3,
                                                                                                                1,
                                                                                                                1))
        ''' Smoothness Loss '''
        loss_h = criterion_L1(M_right_to_left[:,
                                              :-1,
                                              :,
                                              :],
                              M_right_to_left[:,
                                              ...
                                              :]) + criterion_L1(M_left_to_right[:,
                                                                 ...
                                                                 M_left_to_right[:,
                                                                                 ...
                                                                                 :,
                                                                                 :])
        loss_w = criterion_L1(M_right_to_left[:,
                                              :,
                                              :-1,
                                              :-1],
                              M_right_to_left[:,
                                              :,
                                              1:,
                                              1:]) + criterion_L1(M_left_to_right[:,
                                                                                  :,
                                                                                  :-1,
                                                                                  :-1],
                                                                  M_left_to_right[:,
            ...
            Res_leftT.permute(
                ...
                c)).view(
                    ...
                    c).contiguous().permute(
                        ...
                        1,
            2)
        self.loss_cycle = criterion_L1(Res_left * V_left.repeat(1,
                                                                3,
                                                                1,
                                                                1),
                                       Res_left_cycle * V_left.repeat(1,
                                                                      3,
                                                                      1,
                                                                      1)) + criterion_L1(Res_right * V_right.repeat(1,
                                                                                                                    3,
                                                                                                                    1,
                                                                                                                    1),
                                                                                         Res_right_cycle * V_right.repeat(1,
                                                                                                                          3,
                                                                                                                          1,
                                                                                                                          1))
        ''' Consistency Loss '''
        SR_left_res = F.interpolate(
            torch.abs(
                HR_left - SR_left),
            scale_factor=1 / scale,
            mode='bicubic',
        ...
        SR_right_res = F.interpolate(
            ...
            align_corners=False,
            recompute_scale_factor=True)
        SR_left_resT = torch.bmm(
            M_right_to_left.detach().contiguous().view(
                b *
                h,
                w,
                w),
            SR_right_res.permute(
                0,
                2,
                3,
                1).contiguous().view(
                ...
                c)).view(
                    ...
                    c).contiguous().permute(
                        ...
                        1,
            2)
        SR_right_resT = torch.bmm(
            M_left_to_right.detach().contiguous().view(
                b *
                h,
                w,
                w),
            SR_left_res.permute(
                ...
                c)).view(
                    ...
                    c).contiguous().permute(
                        ...
                        1,
            2)
        self.loss_cons = criterion_L1(SR_left_res * V_left.repeat(1,
                                                                  3,
                                                                  1,
                                                                  1),
                                      SR_left_resT * V_left.repeat(1,
                                                                   3,
                                                                   1,
                                                                   1)) + criterion_L1(SR_right_res * V_right.repeat(1,
                                                                                                                    3,
                                                                                                                    1,
                                                                                                                    1),
                                                                                      SR_right_resT * V_right.repeat(1,
                                                                                                                     3,
                                                                                                                     1,
                                                                                                                     1))
        ''' Total Loss '''
    ...
    def flop(self, H, W):
        ...
        flop += 2 * (N * (64 + 1) * 64 * (self.upscale_factor **
                     2) + (N**self.upscale_factor) * 3 * (64 * 9 + 1))
        return flop
