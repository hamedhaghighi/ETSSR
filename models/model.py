...
class Net(BaseModel):
    def __init__(self, upscale_factor, img_size,
                 model, input_channel=3, w_size=8):
        ...
        self.init_feature = nn.Conv2d(
            self.input_channel, 64, 3, 1, 1, bias=True)
        if 'CP' in self.model:
            self.condition_feature = nn.Sequential(
                nn.Conv2d(
                    4, 64, 3, 1, 1, bias=True), nn.LeakyReLU(
                    0.1, inplace=True), nn.Conv2d(
                    64, 64, 3, 1, 1, bias=True))
        self.n_RDB = 3 if 'MDB' in model else 4
        self.deep_feature = RDG(
            G0=64,
            C=4,
            G=24,
            n_RDB=self.n_RDB,
            type='P') if 'MDB' in model else RDG(
            G0=64,
            C=4,
            G=24,
            n_RDB=self.n_RDB,
    ...
    def forward(self, x_left, x_right, is_training=0):
        ...
        if 'seperate' in self.model:
            ...
            x_left_selected, x_right_selected = x_left[coords_b, :, coords_h, l2r_w].permute(
                0, 3, 1, 2), x_right[coords_b, :, coords_h, r2l_w].permute(0, 3, 1, 2)
            x_left, x_right = torch.cat([x_left, x_left_selected], dim=1), torch.cat([
                x_right, x_right_selected], dim=1)

        CF_left = self.condition_feature(
            x_left[:, 3:]) if 'CP' in self.model else None
        CF_right = self.condition_feature(
            x_right[:, 3:]) if 'CP' in self.model else None
        ...
        if 'pam' in self.model:
            if 'light' in self.model:
                buffer_leftT, buffer_rightT = self.swin(
                    buffer_left), self.swin(buffer_right)
                buffer_leftT, buffer_rightT = self.pam(
                    buffer_leftT, buffer_rightT, catfea_left, catfea_right)
            else:
                buffer_leftT, buffer_rightT = self.pam(
                    buffer_left, buffer_right, catfea_left, catfea_right)
        elif 'coswin' in self.model:
            buffer_leftT, buffer_rightT = self.swin(
                buffer_left), self.swin(buffer_right)
            if 'coswin_wo_d' in self.model:
                buffer_leftT, buffer_rightT = self.coswin(
                    buffer_leftT, buffer_rightT)
            else:
                buffer_leftT, buffer_rightT = self.coswin(
                    buffer_leftT, buffer_rightT, d_left, d_right)
        ...
        if 'mine_late_fusion' in self.model:
            ...
            buffer_leftF, buffer_rightF = self.fusion(torch.cat([buffer_left, buffer_leftT], dim=1)), self.fusion(
                torch.cat([buffer_right, buffer_rightT], dim=1))
        ...
        if 'late_fusion' in self.model:
            buffer_leftF, buffer_rightF = self.swin(
                buffer_leftF), self.swin(buffer_rightF)
            buffer_leftF, buffer_rightF = self.coswin(
                buffer_leftF, buffer_rightF, d_left, d_right)
        out_left = self.upscale(buffer_leftF) + x_left_upscale
        out_right = self.upscale(buffer_rightF) + x_right_upscale
        mod_h = -mod_pad_h * \
            self.upscale_factor if (
                ('pam' not in self.model) and mod_pad_h != 0) else None
        mod_w = -mod_pad_w * \
            self.upscale_factor if (
                ('pam' not in self.model) and mod_pad_w != 0) else None
        return out_left[..., :mod_h, :mod_w], out_right[..., :mod_h, :mod_w]

    def flop(self, H, W):
        N = H * W
        flop = 0
        flop += 2 * ((self.input_channel * 9 + 1)
                     * N * 64)  # adding 1 for bias
        if 'CP' in self.model:
        ...
        flop += 2 * (self.f_RDB.flop(N) +
                     self.CAlayer.flop(N) + N * (128 + 1) * 64)
        flop += 2 * self.reconstruct.flop(N)
        flop += 2 * (N * (64 + 1) * 64 * (self.upscale_factor ** 2) +
                     N * (self.upscale_factor ** 2) * (64 * 9 + 1) * 3)
        return flop
...
class PAM(nn.Module):
    ...
    def __call__(self, x_left, x_right, catfea_left,
        ...
        if is_training == 1:
            return out_left, out_right, (M_right_to_left.contiguous().view(
    ...
    def flop(self, H, W):
        ...
        flop += 2 * N * self.channel * \
            (self.n_RDB * self.channel + 1) * (1 / 4)
...
class LightPAM(nn.Module):
    ...
    def __call__(self, x_left, x_right, catfea_left,
        ...
        V_left = torch.bmm(M_right_to_left_relaxed.contiguous().view(-1,
                           ...
                           M_left_to_right.permute(0,
                                                   2,
                                                   1).contiguous().view(-1,
                                                                        w_size).unsqueeze(2)).detach().contiguous().view(b,
                                                                                                                         1,
                                                                                                                         h,
                                                                                                                         w)  # (B*H*Wr) * Wl * 1
        M_left_to_right_relaxed = M_Relax(M_left_to_right, num_pixels=2)
    ...
    def flop(self, H, W):
        ...
        flop += 3 * N * self.channel * \
            (self.n_RDB * self.channel + 1) * (1 / self.g)
...
if __name__ == "__main__":
    ...
    H, W, C = 360, 640, 3
    net = Net(upscale_factor=4, model='rdb_pam', img_size=tuple(
